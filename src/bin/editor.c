/**
 * @file editor.c
 * @brief Vix 3.0 - A Vim-like Text Editor for JexOS.
 *
 * Supports syntax highlighting, vertical scrolling, and integrated
 * compilation/execution of C code.
 */
...
/* Terminal functions provided by the kernel */
...
/**
 * @brief Utility to check if a character is a digit.
 */
int is_digit(char c) {
    return (c >= '0' && c <= '9');
}
...
/**
 * @brief Determine the syntax color for a character at a given position.
 * 
...
uint8_t get_char_color(int pos) {
    ...
    if (!is_alpha_num(cc)) return 0x07; /* Grey */
    
    /* Keywords and Syscalls */
    if (pos == 0 || !is_alpha_num(edit_buffer[pos-1])) {
        ...
        for (int i = 0; syscalls[i]; i++) { if (strcmp(word, syscalls[i]) == 0) return 0x0A; } /* Light Green */

        /* Function calls (heuristic: word followed by '(') */
        ...
        for (int i = 0; keywords[i]; i++) { if (strcmp(word, keywords[i]) == 0) return 0x0B; } /* Cyan */
    } else {
        /* Continue color from the start of the word */
        int start = pos;
...
/**
 * @brief Draw the bottom status bar with file info and shortcuts.
 */
...
void render_text() {
    ...
    int x_offset = 4; /* Line numbers width */
    int x = x_offset, y = 0;
    int line_num = 1;
    char ln_buf[5];

    for (int i = 0; i < edit_len; i++) {
        /* Render only if the line is within the visible window (vertical scroll) */
        if (line_num > edit_scroll_y && y < VISIBLE_HEIGHT) {
            ...
            if (c != '\n') {
                ...
                x++;
                if (x >= EDITOR_WIDTH) { x = x_offset; y++; } /* Line wrap */
            }
        }
        if (edit_buffer[i] == '\n') {
            if (line_num > edit_scroll_y) {
                /* Syntax hint: Warn if line doesn't end with expected symbols (optional) */
...
/**
 * @brief Map logical cursor (x,y) to a byte offset in the edit buffer.
 */
...
/**
 * @brief Write the current edit buffer to disk.
 */
...
/**
 * @brief Handle keyboard input for the editor.
 */
void editor_input(char key) {
    ...
    /* Redraw screen (with optimization for bulk input) */
